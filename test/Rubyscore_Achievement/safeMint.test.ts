...
describe("Method: safeMint: ", () => {
  ...
  describe("When one of parameters is incorrect", () => {
    it("When signer not equal operator", async () => {
      ...
      const notOperatorSignature: string = await sign(domain, types, "MintParams", mintParams, user);
    ...
    it("When user change data", async () => {
      ...
      const notOperatorSignature: string = await sign(domain, types, "MintParams", mintParams, minter);
    ...
    it("When try mint with wrong nonce", async () => {
      ...
      const operatorSignature: string = await sign(domain, types, "MintParams", dataWithWrongNonce, minter);
    ...
    it("When try mint second NFT to one address", async () => {
      ...
      let operatorSignature: string = await sign(domain, types, "MintParams", mintParams, minter);
      ...
      operatorSignature = await sign(domain, types, "MintParams", newBuyParams, minter);
      await expect(
    ...
    it("When try mint with wrong payment amount", async () => {
      ...
      operatorSignature = await sign(domain, types, "MintParams", mintParams, minter);
      await expect(
    ...
    it("When try mint second NFT to one address with batch", async () => {
      ...
      let operatorSignature: string = await sign(domain, types, "MintParams", batchBuyParams, minter);
